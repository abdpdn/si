...
class Hamiltonian {
  // Simulating H = \sum_i (J_i S_i^x S_{i+1}^x + S_i^y S_{i+1}^y + Delta S_i^z S_{i+1}^z - (h_i-U_i) S_i^z)
 private:
  basis *basis_pointer;
  Mat *pointer_to_H;
...
void Hamiltonian::get_parameters() {
  ...
  PetscErrorCode ierr;
  L = 6;
  ierr = PetscOptionsGetInt(NULL, NULL, "-L", &L, NULL);  // PetscCall(ierr);
  int Ldouble = 2 * L;
  ierr = PetscOptionsGetInt(NULL, NULL, "-Ldouble", &Ldouble, NULL);
  L = Ldouble / 2;

  ierr = PetscOptionsGetReal(NULL, NULL, "-J", &J, NULL);  // PetscCall(ierr);
  ierr = PetscOptionsGetReal(NULL, NULL, "-UovL", &UovL, NULL);  // PetscCall(ierr);
  ...
  ierr = PetscOptionsGetBool(NULL, NULL, "-gaussian_field", &gaussian_field,
                             NULL);                            // PetscCall(ierr);
  ierr = PetscOptionsGetBool(NULL, NULL, "-pbc", &pbc, NULL);  // PetscCall(ierr);
  ierr = PetscOptionsGetBool(NULL, NULL, "-comparison", &comparison_matrix, NULL);  // PetscCall(ierr);
  ...
  ierr =
      PetscOptionsGetInt(NULL, NULL, "-seed", &seed, NULL);  // PetscCall(ierr);
  coupling.resize(L, J);
  ...
  if (UovL!=0.0) {
    if (myrank == 0) {
        std::cout << "# Adding a cosine potential with U/L = " << UovL << "\n";
    }
    for (int i = 0; i < L; i++) {
      field[i] -= (UovL *L /2) * cos(2 * M_PI * i / L);
    }
  }
          ...
          for (int r = 0; r < L; ++r) {
            if (coupling[r]) {
              ...
              if (config[rb] == config[rb2]) {
                ...
                // find non-zero off-diagonal terms, but do not set their value.
                int nleft = 0;
                int nright = 0;
                for (int p = 0; p < LA; ++p) {
