...
int main(int argc, char **argv) {
  ...
  for (double &renorm_target : targets) { 
    // new setup
    ...
    while (trys < 7 && diagfailed) {
      // recompute target
      target = renorm_target * (Emaxc - Eminc) + Eminc;
      ...
      ierr = EPSSolve(eps2);  // PetscCall(ierr);

      ierr = EPSGetConverged(eps2, &nconv);
      PetscCall(ierr);
      if (0 == myrank) std::cout << "Solved done. nconv = "<< nconv <<"\n" ;
      if (nconv == 0) {
        std::cout << "No eigenvalue found for target " << target << std::endl;
        if (trys == 0) {
          renorm_target = renorm_target+0.000012;
        }else{
          renorm_target = 0.9999*renorm_target;
        }  
        trys++;
      }else{
        diagfailed = false;
      ...
      for (int i = 0; i < nconv; i++) {
        ...
        if (myrank == 0) {
          ...
          */
          ...
          // VERY important : keep this at the very end of the measurements, because it modifies the state!
